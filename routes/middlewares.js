...
module.exports = function (app) {
    ...
    app.use(function (req, res, next) {
        ...
        // 登入登出和外部访问接口以及angular app入口不采取登陆拦截，直接指向路由
        if (req.originalUrl != "/" && req.originalUrl.split('/')[1] != "api" && req.originalUrl.split('/')[1] != "dev") {
            // api接口强制使用验证DSA开头的内部接口，其余流过到模糊匹配进行错误处理
            if (!req.session.user){
